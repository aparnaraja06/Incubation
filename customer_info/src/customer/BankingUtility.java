...
public class BankingUtility{
	...
	public void getMap(Map<Integer,CustomerInfo> customerDetails,Map<Integer,Map<Integer,AccountInfo>> accountDetails)throws CustomException
	{
	...
	/*public void defaultCustomer() {
		...
		long[] mobile_arr= {984659841L,8130257081L,72359126L,9475819512L,921047598L};
		...
		for(int i=0;i<name_arr.length;i++)
		{
			...
			customer.setMobileNum(mobile_arr[i]);
	...
	public void defaultAccount()throws CustomException
	{
		...
		for(int i=0;i<branch_arr.length;i++)
		{
			...
			checkCustomerExists(customer_id[i]);
	
			AccountInfo account= new AccountInfo();
			int account_id=getAccountNum();
			account.setCustomerId(customer_id[i]);
			...
			accountMap.put(customerId,tempMap);
		}
	...
	public Map<Integer,CustomerInfo> showCustomers() 
	{
		return customerMap;
	...
	public boolean changeStatus(int customer_id,int acc_num,boolean active)throws CustomException
	{
		...
		//System.out.println("Activity before setting :"+tempMap.get(acc_num).isActive());
		AccountInfo accountNum=tempMap.get(acc_num);
